...
BTCKey::getPrivKeySeed(const std::string &phrase, const std::string &phrasePassword,
                       const char* i18nPath, const std::string &language, int nid) {
    CMBlock out;
    if (phrase == "" || i18nPath == "") {
        return out;
    }
    ...
    if (language == "english" || language == "") {
        ...
        char fileName[512];
        strcpy(fileName, i18nPath);
        strcat(fileName, "mnemonic_");
...
std::string BTCKey::getAddressFromPublicKey(const CMBlock& publicKey)
{
